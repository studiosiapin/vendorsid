...
export default function OrderForm() {
    ...
    const validateStep1 = () => {
        ...
        if (!formData.linkMockup)
            newErrors.linkMockup = 'Foto Mockup is required';
        if (!formData.linkCollar)
            newErrors.linkCollar = 'Foto Collar is required';
        if (!formData.linkLayout)
            newErrors.linkLayout = 'Foto Layout is required';
    ...
    const validateStep2 = () => {
        ...
        formData.orderDetails.forEach((orderDetail, index) => {
            ...
            if (!orderDetail.ukuranId) {
                newErrors[`orderDetails[${index}].ukuranId`] =
                    'Ukuran is required';
            }
        });
        setErrors(newErrors);
    ...
    const validateStep3 = () => {
        const newErrors: Partial<Record<string, string>> = {};
        if (!formData.shipmentCode)
            newErrors.shipmentCode = 'Shipment is required';
        ...
        if (formData.dpAmount < formData.totalAmount / 2)
            newErrors.dpAmount =
                'DP Amount must be at least 50% of Total Amount';
        setErrors(newErrors);
        return Object.keys(newErrors).length === 0;
    };
        ...
        if (step === 1) {
            ...
            if (validateStep2()) {
                if (formData.jenisCode && formData.orderDetails.length > 0) {
                    ...
                    const totalAmount = pricePerUnit
